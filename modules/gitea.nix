...
let
  cfg = config.services.gitea-fs;
  opt = options.services.gitea-fs;
  gitea = cfg.package;
...
{
  imports = [
    (mkRenamedOptionModule [ "services" "gitea-fs" "cookieSecure" ] [ "services" "gitea" "settings" "session" "COOKIE_SECURE" ])
    (mkRenamedOptionModule [ "services" "gitea-fs" "disableRegistration" ] [ "services" "gitea" "settings" "service" "DISABLE_REGISTRATION" ])
    (mkRenamedOptionModule [ "services" "gitea-fs" "log" "level" ] [ "services" "gitea" "settings" "log" "LEVEL" ])
    (mkRenamedOptionModule [ "services" "gitea-fs" "log" "rootPath" ] [ "services" "gitea" "settings" "log" "ROOT_PATH" ])
    (mkRenamedOptionModule [ "services" "gitea-fs" "ssh" "clonePort" ] [ "services" "gitea" "settings" "server" "SSH_PORT" ])

    (mkRemovedOptionModule [ "services" "gitea-fs" "ssh" "enable" ] "services.gitea.ssh.enable has been migrated into freeform setting services.gitea.settings.server.DISABLE_SSH. Keep in mind that the setting is inverted")
  ];
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.gitea = {
      ...
      in ''
        ...
        ${gitea}/bin/gitea -c ${runConfig} migrate

        # update all hooks' binary paths
        ${gitea}/bin/gitea -c ${runConfig} admin regenerate hooks

        # update command option in authorized_keys
        if [ -r ${cfg.stateDir}/.ssh/authorized_keys ]
        then
          ${gitea}/bin/gitea -c ${runConfig} admin regenerate keys
        fi
      '';
