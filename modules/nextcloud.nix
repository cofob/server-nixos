...
let
  cfg = config.services.fs-nextcloud;
  ...
  config = mkIf cfg.enable (mkMerge [
    {
      ...
      services.fs-nextcloud.package = with pkgs;
        mkDefault (
          if pkgs ? nextcloud
          then
            throw ''
      ...
      services.fs-nextcloud.phpPackage =
...

    {
      ...
      services.phpfpm = {
        pools.nextcloud = {
          ...
          settings = mapAttrs (name: mkDefault)
            {
              "listen.owner" = config.services.fs-nginx.user;
              "listen.group" = config.services.fs-nginx.group;
            } // cfg.poolSettings;
      ...
      users.groups.nextcloud.members = [ "nextcloud" config.services.fs-nginx.user ];
      ...
      services.fs-nginx.enable = mkDefault true;

      services.fs-nginx.virtualHosts.${cfg.hostName} = {
        root = cfg.package;
        locations = {
          "= /robots.txt" = {
            priority = 100;
