...
let
  ...
  mkCertOwnershipAssertion = import ./mk-cert-ownership-assertion.nix;
in

{
  ...
  imports = [
    (mkRemovedOptionModule [ "services" "fs-nginx" "stateDir" ] ''
  ...
  config = mkIf cfg.enable {
    ...
    users.users = optionalAttrs (cfg.user == "nginx") {
      nginx = {
        ...
        extraGroups = lib.mkIf (config.security.acme.certs != { }) [ "acme" ];
      };
    };
